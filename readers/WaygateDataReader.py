...
class WaygateDataReader(object):
    ...
    def set_up(self, 
        ...
        # #initialise parameters keep code working?
        object_tilt_deg = 0
        ...
        #Parameters not sure where in pca file
        initial_angle = 0

        for line in content:
            # number of projections
            if line.startswith("StartImg"):
                line = line.replace('\"', ' ')
            ...
            elif line.startswith("RotationSector"):
                line = line.replace('\"', ' ')
                final_angle = float(line.split('=')[1])
            elif line.startswith("Level"):
                line = line.replace('\"', ' ')
                self.Level = float(line.split('=')[1])
            # object offset x in units                
            elif line.startswith("XSample"):
                line = line.replace('\"', ' ')
                object_offset_x = float(line.split('=')[1])

        
           
        input_file = (file_name.split('\\')[-1])
        self.proj_name = (input_file.split('.pca')[0]) 
        ...
        angular_step = (final_angle - initial_angle)/(num_projections)
        ...
        object_roll = np.radians(object_roll_deg)

        object_tilt = -np.radians(object_tilt_deg)
    ...
    def read(self):
        ...
        if (self.normalise):
            ...
            ad = ad / np.float32(self.Level) #self.Level
